...
function loadModalContent(button) {
  ...
  if (toShow == "top100") {
    ...
    if (category == "a") {
      ...
      content = `Category B contains courses ranking roughly 20–50 are the "hidden" giants that often rival Category A in quality, if not in Open history. This tier includes the legendary Royal Dornoch, the stunning modern masterpiece Kingsbarns, and the quirky, brilliant North Berwick (West). You will also find Castle Stuart (Cabot Highlands) and Trump International here. These courses are frequently rated among the world's best. They offer world-class layouts and breathtaking scenery but lack the "Major" hosting status, often making them slightly more accessible—though no less challenging or visually spectacular.`;
    } else if (category == "c") {
      content = `Ranking from 50–100, Category C represents the backbone of high-quality Scottish golf. It features storied names like Cruden Bay, The Machrie, and Gullane No. 1, alongside inland gems like Ladybank or the King's Course at Gleneagles. These courses provide a premier experience with professional-grade conditioning but without the premium price tag of the top 20. They often possess more "character" and unique local quirks, offering a truer sense of the Scottish club atmosphere. For many regular visitors, Category C provides the best balance of value and championship-level play.`;
    } else if (category == "d") {
      content = `Category D encompasses hundreds of "hidden gems" and local municipal courses that represent the true soul of Scottish golf. From the nine-hole clifftop beauty of Anstruther to historic town links like Montrose or Brora, these are the courses where the locals play. They are affordable, welcoming, and often situated on naturally stunning land that hasn't been over-engineered. While they may lack the global fame of a St Andrews, they offer a fast-paced, authentic, and "raw" version of the game that is increasingly rare elsewhere in the world.`;
    }
